...
<section id="register">
    ...
    async function onSubmit(e){
        ...
        if(username.length < 3){
            return renderer(registerTemp('Username must be longer than 3 characters'));
        }
        
        if(password.length < 3){
            return renderer(registerTemp('Password must be longer than 3 characters'));
        }

        if(!email){
            return renderer(registerTemp('Enter email'));
        }

        if(password !== repass){
            return renderer(registerTemp('Password isnt the same as repass'));
        }
